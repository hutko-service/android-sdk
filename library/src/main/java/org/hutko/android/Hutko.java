...
public final class Hutko {
    ...
    private final HutkoView hutkoView;

    public Hutko(int merchantId, HutkoView hutkoView) {
        this.merchantId = merchantId;
        this.hutkoView = hutkoView;
    }
    ...
    private void payContinue(final String token, final Checkout checkout, final PayCallback callback) throws java.lang.Exception {
        ...
        if (checkout.action == Checkout.WITHOUT_3DS) {
            Hutko.runInTry(orderChecker, callback);
        } else {
    ...
    private static JSONObject callJson(String path, TreeMap<String, Object> request) throws java.lang.Exception {
        ...
        if (BuildConfig.DEBUG) {
            Log.i("Hutko", "Read: " + jsonOfResponse);
        }
    ...
    private void url3ds(final String token, final Checkout checkout, final PayCallback callback) throws java.lang.Exception {
        ...
        final HutkoView.PayConfirmation confirmation = new HutkoView.PayConfirmation(
                rs.content,
                contentType[0],
                checkout.url,
                ...
                HOST,
                rs.cookie,
                new HutkoView.PayConfirmation.Listener() {
                    @Override
        ...
        sMain.post(new Runnable() {
            @Override
            public void run() {
                hutkoView.confirm(confirmation);
            }
        });
    }
